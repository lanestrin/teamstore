...
export default function ResizablePanel({
  ...
  const updateFromClientX = useCallback(
    (clientX: number) => {
      ...
      const nextLeftWidth = clamp(clientX - rect.left, minLeft, maxLeft);
  ...
  return (
    <div
      ...
      style={panelStyle}
    >
      <section className={[styles.leftPanel, leftClassName ?? ""].join(" ")}>{left}</section>
      ...
      <aside className={[styles.rightPanel, rightClassName ?? ""].join(" ")}>{right}</aside>
    </div>
  );
}
