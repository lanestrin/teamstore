...
export type ArtworkAdjustments = Record<
  string,
...
function getOrCreateAdjustmentWrapper(
  ...
  if (
    ...
    parent.getAttribute(ADJUSTMENT_ATTRIBUTE) === elementId
  ) {
    return parent;
  }
...
export function applyArtworkAdjustment(
  ...
  const element = svgDocument.getElementById(
    editableElement.id,
  );

  if (!element) {
    console.warn(
      `SVG element "#${editableElement.id}" was not found.`,
    );
    return;
  }

  const constrainedAdjustment =
  ...
  if (
    constrainedAdjustment.x === 0 &&
    constrainedAdjustment.y === 0
  ) {
    return;
  }

  const wrapper = getOrCreateAdjustmentWrapper(
    svgDocument,
    element,
    editableElement.id,
  );
  ...
  wrapper.setAttribute(
    "transform",
    `translate(${constrainedAdjustment.x} ${constrainedAdjustment.y})`,
  );
}
...
export function applyArtworkAdjustments(
  ...
  for (const editableElement of editableElements) {
    const adjustment =
      adjustments[editableElement.id];

    if (!adjustment) {
      continue;
    }

    applyArtworkAdjustment(
      svgDocument,
      editableElement,
      adjustment,
    );
