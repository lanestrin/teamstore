...
function measureSvgText(element: Element, value: string) {
  ...
  const fontSize = parseSvgNumber(getPresentationAttribute(element, "font-size"), 16);
  const fontFamily = getPresentationAttribute(element, "font-family") ?? "sans-serif";
  const letterSpacing = parseSvgNumber(getPresentationAttribute(element, "letter-spacing"), 0);
...
function getTextFitWidth(svgDocument: Document, element: Element, textTarget: Element, originalValue: string) {
  const configuredWidth = textTarget.getAttribute("data-fit-width") ?? element.getAttribute("data-fit-width");

  if (configuredWidth) {
    return parseSvgNumber(configuredWidth, 0);
  }

  const existingTextLength = textTarget.getAttribute("textLength") ?? element.getAttribute("textLength");
...
function getSvgTextTargets(element: Element) {
  const configuredTargets = Array.from(element.querySelectorAll("tspan[data-fit-width], textPath[data-fit-width], text[data-fit-width]"));
  ...
  if (textElements.length > 0) {
    return textElements.map((textElement) => textElement.querySelector("tspan") ?? textElement.querySelector("textPath") ?? textElement);
  }

  return [element.querySelector("tspan") ?? element.querySelector("textPath") ?? element];
}

function getSvgTextFitMode(element: Element, textTarget: Element): SvgTextFitMode {
  const configuredMode = textTarget.getAttribute("data-fit-mode") ?? element.getAttribute("data-fit-mode") ?? "squeeze";
  ...
  switch (configuredMode) {
    ...
    default:
      console.warn(`Unsupported SVG text fit mode "${configuredMode}". Falling back to "squeeze".`);
      return "squeeze";
  }
}

function applySvgTextFit(svgDocument: Document, element: Element, textTarget: Element, originalValue: string, value: string) {
  const fitWidth = getTextFitWidth(svgDocument, element, textTarget, originalValue);
  ...
  const originalFontSize = parseSvgNumber(getPresentationAttribute(textTarget, "font-size"), 16);
  const configuredMinimumFontSize = textTarget.getAttribute("data-fit-min-font-size") ?? element.getAttribute("data-fit-min-font-size");
  ...
  const fittedFontSize = Math.max(minimumFontSize, originalFontSize * (fitWidth / updatedWidth));
...
function setSvgText(svgDocument: Document, elementId: string, value: string) {
  ...
  for (const textTarget of textTargets) {
    ...
    applySvgTextFit(svgDocument, element, textTarget, originalValue, value);
  }
}

function setSvgMascot(svgDocument: Document, mascotElementId: string | undefined, mascotSource: string | null) {
  ...
  for (const attribute of ["x", "y", "width", "height", "transform", "clip-path"]) {
...
function getTextBindingValue(values: ArtworkTextDraft, binding: ArtTemplateTextBinding) {
...
export function createCustomizedSvg(template: ArtTemplate, values: ArtworkTextDraft, mascotSource: string | null) {
  ...
  for (const binding of template.textBindings) {
    setSvgText(svgDocument, binding.elementId, getTextBindingValue(values, binding));
  }
