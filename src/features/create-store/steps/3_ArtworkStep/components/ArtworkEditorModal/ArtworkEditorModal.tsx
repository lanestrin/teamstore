...
import { LuArrowDown, LuArrowLeft, LuArrowRight, LuArrowUp, LuRotateCcw, LuX } from "react-icons/lu";
...
function createEditorSvg(
  ...
  for (const editableElement of editableElements) {
    ...
    element.setAttribute("data-artwork-editor-movement", editableElement.movement);
...
function screenPointToSvgPoint(svg: SVGSVGElement, clientX: number, clientY: number): SvgPoint | null {
...
function getElementAdjustment(adjustments: ArtworkAdjustments, elementId: string): ArtworkElementAdjustment {
  return (
...
export default function ArtworkEditorModal(props: ArtworkEditorModalProps) {
  ...
  return <ArtworkEditorModalContent {...props} />;
}
...
function ArtworkEditorModalContent({ svg, editableElements, adjustments, onCancel, onSave }: ArtworkEditorModalProps) {
  ...
  const [draftAdjustments, setDraftAdjustments] = useState<ArtworkAdjustments>(() => cloneAdjustments(adjustments));
  ...
  const selectedElement = editableElements.find((element) => element.id === selectedElementId);
  ...
  const editorSvg = useMemo(
    () => createEditorSvg(svg, editableElements, draftAdjustments, selectedElementId),
  ...
  const updateSelectedAdjustment = (deltaX: number, deltaY: number) => {
    ...
    setDraftAdjustments((current) => {
      const currentAdjustment = getElementAdjustment(current, selectedElement.id);
  ...
  const handlePreviewPointerDown = (event: ReactPointerEvent<HTMLDivElement>) => {
    ...
    const elementId = editableTarget.getAttribute("data-artwork-editor-element");

    if (!elementId) {
      return;
    }

    const editableElement = editableElements.find((element) => element.id === elementId);
    ...
    const startPoint = screenPointToSvgPoint(svgElement, event.clientX, event.clientY);
  ...
  const handlePreviewPointerMove = (event: ReactPointerEvent<HTMLDivElement>) => {
    ...
    const editableElement = editableElements.find((element) => element.id === dragState.elementId);
    ...
    const currentPoint = screenPointToSvgPoint(svgElement, event.clientX, event.clientY);
    ...
    const nextAdjustment = constrainArtworkAdjustment(
      {
        x: dragState.startAdjustment.x + currentPoint.x - dragState.startPoint.x,
        y: dragState.startAdjustment.y + currentPoint.y - dragState.startPoint.y,
      },
  ...
  const canMoveVertically = selectedElement?.movement === "vertical" || selectedElement?.movement === "both";
        ...
        <div className={styles.body}>
          ...
          <aside className={styles.sidebar}>
            ...
            <p className={styles.position}>
              X: {Math.round(selectedAdjustment.x)} · Y: {Math.round(selectedAdjustment.y)}
            </p>

            <button type="button" className={`${styles.button} ${styles.resetAll}`} onClick={resetAllAdjustments}>
              Reset All Positions
            </button>
          </aside>
        </div>
