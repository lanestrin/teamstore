export type DecorationProfileId =
  | "upper-front"
  | "left-leg"
  | "cap-front";
...
export const DECORATION_PROFILES: Record<
  DecorationProfileId,
  DecorationProfile
> = {
  /*
...
export function getDecorationProfile(
  profileId: DecorationProfileId,
): DecorationProfile {
  return DECORATION_PROFILES[profileId];
}

export function createDefaultProductArtworkPlacement(
  profileId: DecorationProfileId,
): ProductArtworkPlacement {
  const placement =
    DECORATION_PROFILES[profileId].defaultPlacement;
