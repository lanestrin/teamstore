...
export function createProductArtworkSvg({
  ...
  const template =
    ART_TEMPLATE_LIST.find(
      (candidate) =>
        candidate.id === artworkTemplateId,
    );

  if (!template) {
    console.warn(
      `Artwork template "${artworkTemplateId}" was not found.`,
    );
