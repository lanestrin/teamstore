...
export default function RecentOrders({ orders, onViewOrders }: RecentOrdersProps) {
    ...
    <section className={styles.card}>
      ...
      {hasOrders ? (
        <>
            ...
            {orders.map((order) => (
              <li key={order.id} className={styles.order}>
                <div className={styles.image}>
                  <img src={order.image} alt={order.storeName} />
                </div>

                <div className={styles.details}>
                  <h3>Order #{order.orderNumber}</h3>

                  <p className={styles.store}>{order.storeName}</p>

                  <p className={styles.date}>{order.orderDate}</p>
                </div>

                <div className={styles.summary}>
                  <span className={`${styles.status} ${order.status === "Delivered" ? styles.delivered : styles.shipped}`}>
                    {order.status}
                  </span>
                ...
                <button type="button" className={styles.chevron} aria-label={`View order ${order.orderNumber}`}>
                  <LuChevronRight />
                </button>
              </li>
            ))}
          ...
          <button type="button" className={styles.viewAll} onClick={onViewOrders}>
            View All Orders
          </button>
        </>
      ) : (
